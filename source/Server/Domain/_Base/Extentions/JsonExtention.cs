...
namespace Domain._Base.Extentions
{
    public static class JsonExtention
    {
        ...
        {
            if (((IDictionary<string, JToken>) source).ContainsKey("and") || ((IDictionary<string, JToken>) source).ContainsKey("or"))
            {
                var key = ((IDictionary<string, JToken>) source).ContainsKey("and") ? "and" : "or";
            ...
            {
                return ((IDictionary<string, JToken>) source).ContainsKey("dataDb.status");
            }
        }
    }
}
